...
DEFAULT_INPUT_CSV = Path(__file__).resolve().parents[1] / "test_data" / "instance_form_long_comp1_form_comp2_form.csv"
...
def read_words(input_csv):
    ...
    with input_path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        required_headers = {"instance_form_long", "comp1_form", "comp2_form"}

        if not required_headers.issubset(input_headers):
            raise ValueError(
                f"Input CSV must contain instance_form_long, comp1_form, and comp2_form columns: {input_csv}"
            )
        ...
        for row in reader:
            ...
            if instance_form_long and comp1_form and comp2_form:
                phrase_rows.append(
                    {
...
def main():
    ...
    parser.add_argument(
        "output_csv",
        nargs="?",
        default="classified_nouns_2_3.csv",
        help="Output CSV file, e.g. classified_nouns_2_3.csv"
    )

    parser.add_argument(
        "--input_csv",
        default=DEFAULT_INPUT_CSV,
        help="Input CSV file with instance_form_long, comp1_form, and comp2_form columns"
    )
