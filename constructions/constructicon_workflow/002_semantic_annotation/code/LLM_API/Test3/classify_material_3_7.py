...
DEFAULT_INPUT_CSV = Path(__file__).resolve().parents[1] / "classified_nouns_3_2_zero_values.csv"
...
def read_words(input_csv):
    ...
    with input_path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            instance_form = (row.get(input_header) or "").strip()
...
def main():
    ...
    parser.add_argument(
        "output_csv",
        nargs="?",
        default="classified_materials_3_7.csv",
        help="Output CSV file, e.g. classified_materials_3_7.csv"
    )

    parser.add_argument(
        "--input_csv",
        default=str(DEFAULT_INPUT_CSV),
        help="Input CSV file generated from classified_nouns_3_2.csv zero values"
    )
    ...
    todo_rows = [
        ...
        if phrase_row["instance_form"] not in done_words
    ]
