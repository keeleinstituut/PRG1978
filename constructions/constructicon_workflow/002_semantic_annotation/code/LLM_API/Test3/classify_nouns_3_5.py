...
def read_words(input_csv):
    ...
    with input_path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        if reader.fieldnames and {"instance_form_long", "sentence"}.issubset(reader.fieldnames):
            for row in reader:
                instance_form_long = (row.get("instance_form_long") or "").strip()
                sentence = (row.get("sentence") or "").strip()

                if instance_form_long and sentence:
                    phrase_rows.append(
                        {
                            "instance_form_long": instance_form_long,
                            "sentence": sentence,
                        }
                    )
    ...
    raise ValueError(
        f"Input CSV must contain instance_form_long and sentence columns: {input_csv}"
    )
    ...
    with output_path.open("r", encoding="utf-8", newline="") as f:
        ...
        for row_idx, row in enumerate(reader):
            ...
            if len(row) >= 3:
                ...
                if word and sentence and answer in {"0", "1"}:
                    done.add((word, sentence))
...
def init_output_file(output_csv, overwrite):
    ...
    if not output_path.exists():
        with output_path.open("w", encoding="utf-8", newline="") as f:
            ...
            writer.writerow(["instance_form_long", "sentence", "0/1"])
...
def call_openrouter(api_key, model_name, phrase_row, timeout, max_retries):
    instance_form_long = phrase_row["instance_form_long"]
    sentence = phrase_row["sentence"]
    user_prompt = f"""Analüüsitav fraas: "{instance_form_long}"
...
def append_answer(output_csv, word, sentence, answer):
    with open(output_csv, "a", encoding="utf-8", newline="") as f:
        writer = csv.writer(f, delimiter=";")
        writer.writerow([word, sentence, answer])
        f.flush()
...
def main():
    ...
    todo_rows = [
        ...
        if (phrase_row["instance_form_long"], phrase_row["sentence"]) not in done_words
    ]
    ...
    for idx, phrase_row in enumerate(todo_rows, start=1):
        word = phrase_row["instance_form_long"]
        sentence = phrase_row["sentence"]
        ...
        try:
            ...
            append_answer(args.output_csv, word, sentence, answer)
