...
DEFAULT_INPUT_CSV = Path(__file__).resolve().parents[1] / "test_data" / "instance_form_long_sentence.csv"
...
def read_words(input_csv):
    ...
    with input_path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        required_fields = {"instance_form_long", "sentence"}
...
def append_answer(output_csv, input_values, answer):
    with open(output_csv, "a", encoding="utf-8", newline="") as f:
        writer = csv.writer(f, delimiter=";")
        writer.writerow(input_values + [answer])
        f.flush()
...
def main():
    ...
    parser.add_argument(
        "output_csv",
        nargs="?",
        default="classified_nouns_3_5.csv",
        help="Output CSV file, e.g. classified_nouns_3_5.csv"
    )

    parser.add_argument(
        "--input_csv",
        default=str(DEFAULT_INPUT_CSV),
        help="Input CSV file with phrase and sentence columns"
    )
    ...
    for idx, phrase_row in enumerate(todo_rows, start=1):
        ...
        try:
            ...
            append_answer(args.output_csv, phrase_row["input_values"], answer)
