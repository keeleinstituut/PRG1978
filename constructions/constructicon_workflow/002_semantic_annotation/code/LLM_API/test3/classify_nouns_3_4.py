...
DEFAULT_INPUT_CSV = Path(__file__).resolve().parents[1] / "test_data" / "instance_form_long.csv"
...
def read_words(input_csv):
    ...
    with input_path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            instance_form_long = (row.get(input_header) or "").strip()
...
def main():
    ...
    parser.add_argument(
        "output_csv",
        nargs="?",
        default="classified_nouns_3_4.csv",
        help="Output CSV file, e.g. classified_nouns_3_4.csv"
    )

    parser.add_argument(
        "--input_csv",
        default=str(DEFAULT_INPUT_CSV),
        help="Input CSV file with one phrase per row"
    )
    ...
    todo_rows = [
        ...
        if phrase_row["instance_form_long"] not in done_words
    ]
